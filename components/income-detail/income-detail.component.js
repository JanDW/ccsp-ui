  ...
  controller: [
    ...
    function IncomeDetailController($uibModal) {
      ...
      $ctrl.calculateColumns = function() {
        ...
        if ($ctrl.employee.salary) {
          incomes.push($ctrl.employee.salary);
        }
        if ($ctrl.employee.additionalIncome) {
          incomes.push($ctrl.employee.additionalIncome);
        }
        if ($ctrl.spouse && $ctrl.spouse.salary) {
          incomes.push($ctrl.spouse.salary);
        } // check if spouse exists first, checking property on undefined obj would throw err
        if ($ctrl.spouse && $ctrl.spouse.additionalIncome) {
          incomes.push($ctrl.spouse.additionalIncome);
        }
      ...
      $ctrl.editIncome = function() {
        $uibModal
          .open({
            template:
              '<income-detail-edit employee="$ctrl.employee" spouse="$ctrl.spouse" $close="$close(result)" $dismiss="$dismiss(reason)"></income-detail-edit>',
            controller: [
              'employee',
              'spouse',
              function(employee, spouse) {
                let $ctrl = this;
          ...
          .result.then(
            function(result) {
              ...
              if (typeof result.spouse === 'undefined') {
                result.spouse = {};
              }
              // Update total income with new values
              $ctrl.totalIncome =
                result.employee.salary +
                (result.employee.additionalIncome || 0) +
                (result.spouse.salary || 0) +
                (result.spouse.additionalIncome || 0);
