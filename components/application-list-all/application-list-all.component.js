angular.module('applicationListAll').component('applicationListAll', {
  templateUrl:
    'components/application-list-all/application-list-all.template.html',
  controller: [
    '$http',
    '$window',
    '$timeout',
    ...
    'DTColumnDefBuilder',
    '$uibModal',
    function ApplicationListAllController(
      $http,
      $window,
      $timeout,
      DTOptionsBuilder,
      ...
      $window.document.title =
        'All Applications | MIT Childcare Center Scholarship';

      $ctrl.dtOptions = DTOptionsBuilder.newOptions().withDisplayLength(100);

      // Get all applications with statusCode 0 corresponds to (statusMessage:'Pending approval')
      $http
        .get(
          'https://ccsp-api.herokuapp.com/applications?_expand=employee&_embed=awards&_sort=lastSubmissionDate&_order=desc'
        )
        .then(function(response) {
          ...
          $timeout(function() {
            angular
              .element(document)
              .find('#applicationsTable_filter')
              .after(filterTimeTemplate);
            angular
              .element(document)
              .find('.container[ng-view]')
              .addClass('applications-container');
          }, 250);
          ...
          $ctrl.createApplication = function() {
            $uibModal
              .open({
                template:
                  '<application-create $close="$close(result)" $dismiss="$dismiss(reason)"></application-create>',
                controller: [
                  function() {
                    let $ctrl = this;
                  },
          ...
          angular.forEach($ctrl.applications, function(application) {
            switch (application.statusCode) {
              ...
              default:
                console.log(
                  "Wasn't able to select a badge based on statusCode"
                );
            }
