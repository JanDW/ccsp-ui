...
angular
  ...
  .component('financialDocumentsDetail', {
    templateUrl:
      'components/financial-documents-detail/financial-documents-detail.template.html',
    bindings: {
      ...
      function FinancialDocumentsDetailController($uibModal) {
        ...
        $ctrl.editFinancialDocuments = function() {
          $uibModal
            .open({
              template:
                '<financial-documents-edit documents="$ctrl.documents" $close="$close(result)" $dismiss="$dismiss(reason)"></financial-documents-edit>',
              controller: [
                'documents',
                function(documents) {
                  let $ctrl = this;
