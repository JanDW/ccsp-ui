/* jshint esversion:6 */
angular.module('spouseDetail').component('spouseDetail', {
    ...
    function SpouseDetailController($uibModal) {
      ...
      $ctrl.editSpouse = function() {
        $uibModal
          .open({
            template:
              '<spouse-detail-edit employee="$ctrl.employee" spouse="$ctrl.spouse" $close="$close(result)" $dismiss="$dismiss(reason)"></spouse-detail-edit>',
            controller: [
              'employee',
              'spouse',
