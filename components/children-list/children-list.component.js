/* jshint esversion:6 */
angular.module('childrenList').component('childrenList', {
    ...
    function ChildrenListController($uibModal) {
      ...
      $ctrl.editChildrenList = function() {
        ...
        $uibModal
          .open({
            template:
              '<children-list-edit children="$ctrl.children" $close="$close(result)" $dismiss="$dismiss(reason)"></children-list-edit>',
            controller: [
              'children',
              function(children) {
                let $ctrl = this;
