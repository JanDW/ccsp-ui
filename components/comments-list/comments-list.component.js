angular.module('commentsList').component('commentsList', {
    ...
    function CommentsListController($uibModal) {
      ...
      $ctrl.addComment = function() {
        $uibModal
          .open({
            template:
              '<comments-list-add application="$ctrl.application" $close="$close(result)" $dismiss="$dismiss(reason)"></comments-list-add>',
            controller: [
              'application',
